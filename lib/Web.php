...
<div class='cssbox'>
 ...
 function sqlviewDetails($id) {
    ...
    $sql = "select *, (select count(*) from views where parent='$id') as childcount from views where id='$id'";
    $res = mysql_query( $sql );
    ...
    $f = new Form( array( "name"=>"infoForm", "action"=>"$cfg->backEndPath/prb_be.php", "method"=>"get" ));
    $f->frmOpenFieldset( array( "name"=>"details", "legend"=>"Details", ));
    ...
    if( $view->status == 'view' ) {
        $html .= "<div id=graphs>";
        $html .= $this->showGraphs($view->query);
        $html .= "</div>";
    }
    return $html;
 ...
 function showGraphs($sql) {
        ...
        if( $sql != '' ) {
            ...
            while( $info = mysql_fetch_assoc($res) ) {
                ...
                $html .= "<p><b>$m->name $m->description</b><br>\n";
