 ...
 function activateAjaxForm(selector, output) {
    var submits = $ES('form[id='+selector+'] button');
    if (submits.length) {
        submits.addEvent('click', function(e) {
            new Event(e).stop();
            if(window.ie){
            ...
            var out = $(output).addClass('ajax-loading');
            ...
            thisForm.setStyle('display', 'none');
            thisForm.send({
                method: 'post',
                evalScripts: true,
                update: out,
...
/*
 function activateAjaxButton(selector, output) {
    $(selector).addEvent('click', function(e) {
        new Event(e).stop(); //dont submit
        var out = $(output).addClass('ajax-loading');
        var url = $(selector).getProperty('href');
        new Ajax(url, {
            evalScripts: true,
            method: 'get',
            update: out,
