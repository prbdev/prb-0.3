...
#
 ...
 switch( $action ) {
    ...
    case 'snmpdiscovery':
            ...
            $html  = "<script src='js/functions.js' type='text/javascript' language='JavaScript'></script>";
            $html .= "<table>";
            $html .= "<tr><th>select</th><th>ifIndex</th><th>rec name</th><th>ifDescr</th><th>ifAlias</th><th>ifType</th><th>Status</th></tr>";
            for($i=0; $i<count($ifInfo); $i++) {
                $html .= showIfInfo($ifInfo[$i], $i);
            }
            $html .= "</table>";
            $html .= "<div style='cursor: hand;' id=checkall  onClick=\"setCheckboxes('tablesForm', true); return false;\">Check all</div>";
            $html .= "<div style='cursor: hand;' id=checknone onClick=\"setCheckboxes('tablesForm', false); return false;\">Uncheck all</div>";
