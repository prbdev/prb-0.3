...
#
 ...
 switch( $action ) {
    ...
    case 'updatefolder': 
        ...
        $form = $_REQUEST[form];
        $sql = "update views set `name`='$form[name]', `description`='$form[description]', `query`='$form[query]'
                where id='$viewid'";
        mysql_query($sql) || $html = mysql_error();
        $html  = "<script>$('$viewid').innerHTML='".$form[name]."';</script>";
    ...
    case 'savefolder':
        ...
        $sql = "insert into views (`name`, `parent`, `status`, `query`) values('$folderForm[name]', '$folderForm[parent]', '$folderForm[status]', '$folderForm[query]');";
        mysql_query($sql);
	$id = mysql_insert_id();
    ...
    case 'delete':
        if( $infoid!='' ) {
            $html  = "<span class=warning>This will permanently delete this module from the database!</span><p />";
            $html .= $w->confirmDeleteForm('', $infoid);
        } elseif( $host!='' ) {
            $html  = "<span class=warning>This will permanently delete this host and related data from the database!</span><p />";
